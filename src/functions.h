...
{
  if (*numberOfSets < MAX_SETS_NUMBER - 1)
    return true;
...
{
  if (!existSet(sets))
    return message("Nenhum conjunto encontrado!");
  printSets(sets);
  enterToContinue();
}
...
{
  if (!canUnionSets(sets))
    return message("Numero de conjuntos insuficiente!");
  ...
  for (int setIndex = 0; setIndex < newSetMaxLength; setIndex++)
  {
    ...
    else
    {
      ...
      {
        if (newSet[column] != setB[setIndex - setALength])
          continue;
...
{
  if (canAddNewSet(&sets->counter))
    unionOrIntersection(true, sets);
}

void intersectSets(struct setsAttributes *sets)
{
  if (canAddNewSet(&sets->counter))
    unionOrIntersection(false, sets);
}
